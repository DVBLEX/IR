...
import ie.irishrail.server.base.entity.SmsScheduler;
...
public interface SmsSchedulerRepository extends CrudRepository<SmsScheduler, Long> {
    @Query("SELECT ss.id, ss.type, ss.config_id, ss.account_id, ss.template_id, ss.msisdn, ss.source_addr, ss.message,"
    ...
    @Modifying
    @Query("INSERT INTO sms_scheduler(id, is_processed, type, config_id, account_id, template_id, priority, msisdn, source_addr, message, channel, "
            + "date_created, date_scheduled, retry_count, date_processed, transaction_id, response_code, response_text) SELECT id, is_processed, type, config_id, account_id, template_id, priority, "
