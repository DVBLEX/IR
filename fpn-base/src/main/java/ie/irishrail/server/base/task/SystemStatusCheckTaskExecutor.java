...
public class SystemStatusCheckTaskExecutor implements Runnable {
    ...
    private SystemStatusCheckTimerTask.Data data;
    ...
    public SystemStatusCheckTaskExecutor(JdbcTemplate jdbcTemplate, long id, String name, String query, String queryParam, String configParam, int comparisonOperator,
        int comparisonValue, SystemStatusCheckTimerTask.Data data) {
    ...
    private void callServlet() {
        ...
        try {
            ...
            RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(configParamArray[0]).setConnectTimeout(configParamArray[1]).setConnectionRequestTimeout(
                configParamArray[2]).build();
            ...
            log.error("callServlet#id=" + id + ", name=" + name + "###ConnectException: ", ce);
            ...
            log.error("callServlet#id=" + id + ", name=" + name + "###SocketException: ", se);

        } catch (SocketTimeoutException ste) {
            data.finishTime = System.currentTimeMillis();
            data.queryResult = 1005;
            log.error("callServlet#id=" + id + ", name=" + name + "###SocketTimeoutException: ", ste);

        } catch (ConnectTimeoutException cte) {
            data.finishTime = System.currentTimeMillis();
            data.queryResult = 1006;
            log.error("callServlet#id=" + id + ", name=" + name + "###ConnectTimeoutException: ", cte);

        } catch (Exception e) {
            data.finishTime = System.currentTimeMillis();
            data.queryResult = 1000;
            log.error("callServlet#id=" + id + ", name=" + name + "###Exception: ", e);
        } finally {
            try {
                if (httpClient != null) {
                    httpClient.close();
                ...
                log.error("callServlet#id=" + id + ", name=" + name + "###Exception[finally]: ", e);
            }
        }
    }
    ...
    private void processData() {
        ...
        log.info("run#finish#id=" + id + ", name=" + name + ", queryResult=" + data.queryResult + ", comparisonResult=" + data.comparisonResult + ", queryExecutionTime="
