...
import ie.irishrail.server.base.entity.SmsLog;
import ie.irishrail.server.base.repository.SmsLogRepository;
import ie.irishrail.server.base.repository.SmsSchedulerRepository;
...
import ie.irishrail.server.base.repository.SmsConfigRepository;
import ie.irishrail.server.base.repository.SmsTemplateRepository;
...
import ie.irishrail.server.base.common.MessageFormatterUtil;
import ie.irishrail.server.base.common.ServerConstants;
import ie.irishrail.server.base.entity.Sms;
import ie.irishrail.server.base.entity.SmsConfig;
...
public class SMSService {
    ...
    @Transactional
    public void sendBulkSms(Sms sms) throws FPNException {
        ...
        try {
            ...
            if (responseText == null)
                throw new FPNException(ServerResponseConstants.EXTERNAL_API_CONNECTION_FAILURE_CODE, ServerResponseConstants.EXTERNAL_API_CONNECTION_FAILURE_TEXT, "sendBulkSms");
            else {
                ...
                if (sms.getResponseCode() != ServerResponseConstants.SUCCESS_CODE)
                    throw new FPNException(sms.getResponseCode(), sms.getResponseText(), "sendBulkSms");
            }
        } catch (FPNException fpne) {
            throw fpne;
        } catch (Exception e) {
            log.error("sendBulkSms###Exception: ", e);
            throw new FPNException(ServerResponseConstants.EXTERNAL_API_CONNECTION_FAILURE_CODE, e.getClass() + "###" + e.getMessage(), "sendBulkSms###Exception");
        }
    }

    @Transactional
    public void scheduleSmsById(Sms sms, long templateId, HashMap<String, Object> params) throws FPNException {
    ...
    @Transactional
    public void scheduleSmsByType(Sms sms, long templateType, HashMap<String, Object> params) throws FPNException {
    ...
    private long createSmsLog(Sms sms) {
        ...
        smsLog.setAccountId(sms.getAccountId());
    ...
    private String callAllPointsServlet(long smsId, String url, List<NameValuePair> nameValuePairs) throws FPNException {
        ...
        try {
            ...
            if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                throw new FPNException(ServerResponseConstants.EXTERNAL_API_CONNECTION_FAILURE_CODE, responseSource + "Response: [StatusCode="
                    + httpResponse.getStatusLine().getStatusCode() + ", ReasonPhrase=" + httpResponse.getStatusLine().getReasonPhrase() + "]", responseSource);
            }

        } catch (FPNException rbse) {
            throw rbse;

        } catch (Exception e) {
            log.error(responseSource + "smsId=" + smsId + "###Exception: ", e);
            throw new FPNException(ServerResponseConstants.EXTERNAL_API_CONNECTION_FAILURE_CODE, e.getClass() + "###" + e.getMessage(), responseSource + "##Exception");
